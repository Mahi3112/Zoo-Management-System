...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 04, 2024 at 12:48 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddDiagnosis` (IN `diagnosis_id_param` INT, 
IN `animal_id_param` INT, IN `disease_param` VARCHAR(100), IN `medicine_param` VARCHAR(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddEmployee` (IN `p_employee_id` INT,
 IN `p_employee_name` VARCHAR(255), IN `p_job_type` VARCHAR(100), IN `p_employee_salary` DECIMAL(10,2), 
 IN `p_employee_manager` VARCHAR(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddEvent` (IN `event_name` VARCHAR(100), IN `event_date` DATE, 
IN `animals_required` INT, IN `capacity` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddFood` (IN `food_id_param` INT, IN `food_name_param` VARCHAR(100), 
IN `food_type_param` VARCHAR(100), IN `price_param` DECIMAL(10,2), IN `supplier_id_param` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddHealthStatus` (IN `p_health_id` INT, IN `p_animal_id` INT,
 IN `p_last_checkup_date` DATE, IN `p_next_checkup_date` DATE, IN `p_vet_id` INT, IN `p_health` VARCHAR(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `CalculateAndInsertRevenue` (`NoAdult` INT, `NoChildren` INT, 
`AdultUnitPrice` DECIMAL(10,2), `ChildUnitPrice` DECIMAL(10,2)) RETURNS INT(11)  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetMostUnhealthyAnimalType` () RETURNS VARCHAR(100) CHARSET utf8mb4 
COLLATE utf8mb4_general_ci  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `HealthyAnimalPercentage` () RETURNS DECIMAL(10,2)  BEGIN
    ...
    SELECT COUNT(*) INTO healthy_count FROM tblanimal a INNER JOIN health_status hs 
    ON a.ID = hs.animal_id WHERE hs.health = 'Healthy';
...
CREATE DEFINER=`root`@`localhost` FUNCTION `MostCommonDisease` () RETURNS VARCHAR(100)
 CHARSET utf8mb4 COLLATE utf8mb4_general_ci  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `PlaceOrderAndUpdateExpense` (`pItemID` INT, `pQuantity` INT, 
`pItemName` VARCHAR(100)) RETURNS INT(11)  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `UnhealthyAnimalPercentage` () RETURNS DECIMAL(10,2)  BEGIN
    ...
    SELECT COUNT(*) INTO unhealthy_count FROM tblanimal a INNER JOIN health_status hs ON a.ID = hs.animal_id 
    WHERE hs.health = 'Unhealthy';
...
CREATE DEFINER=`root`@`localhost` FUNCTION `UpdateExpenditureForSalaryChange` (`pEmployeeID` INT,
 `pNewSalary` DECIMAL(10,2)) RETURNS INT(11)  BEGIN
    ...
    SELECT CONCAT('Salary for ', employee_name) INTO expenseName FROM employees 
...
(50, 'soham', 'Sanitation worker', 5, 'admin');
...
(13, 'Order for Chicken', 30000, '2024-03-26'),
(14, 'Order for Meat', 100000, '2024-03-26'),
(15, 'Order for Meat', 100000, '2024-03-27'),
(16, 'Salary for Employee ID 50', 5, '2024-03-27'),
(17, 'Order for Chicken', 25000, '2024-03-31');
...
(29, 19, '2024-01-01', '2024-04-04', 11, 'Healthy');
...
(26, 5, 3, 500, '2024-03-26', 'Chicken'),
(27, 1, 3, 500, '2024-03-26', 'Meat'),
(28, 1, 3, 500, '2024-03-27', 'Meat'),
(29, 4, 3, 100, '2024-03-31', 'Chicken');
...
(13, 'Ticket Sales', 300, '0000-00-00'),
(14, 'Ticket Sales', 80, '0000-00-00'),
(15, 'Ticket Sales', 1100, '0000-00-00'),
(16, 'Ticket Sales', 300, '0000-00-00'),
(17, 'Ticket Sales', 1100, '0000-00-00');
...
(1, 'Anantara', 'admin', 1234567890, 'anantaramsd@gmail.com', 'f925916e2754e5e03f75dd58a5733251', '2024-01-31 16:08:00');
...
(10, '243094518', 'Malav', 1, 0, '1100', '800', '2024-03-27 09:58:54'),
(11, '139771545', 'abc', 1, 0, '1100', '800', '2024-03-31 03:47:16');
...
INSERT INTO `tblticindian` (`ID`, `TicketID`, `visitorName`, `NoAdult`, `NoChildren`, `AdultUnitprice`, `ChildUnitprice`, `PostingDate`, `email_id`) VALUES
...
(12, '861487644', 'Mahi Patel', 1, 0, '300', '80', '2024-03-26 11:04:59', ''),
(13, '545629451', 'soham', 0, 1, '300', '80', '2024-03-27 09:51:33', ''),
(14, '918633943', 'Simon Shah', 1, 0, '300', '80', '2024-03-27 09:59:13', '');
...
--
ALTER TABLE `health_status`
  MODIFY `health_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=30;
...
--
ALTER TABLE `revenue`
  MODIFY `revenue_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=18;
